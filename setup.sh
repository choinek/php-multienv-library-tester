...
configure_flags_menu() {
   eheader "Flags Menu" 76
   local FLAG_CONFIG_FILE=".php-multienv-library.config"

   FLAGS=(
      "PMLTC_FLAG_AUTO_UPDATE|Auto Update Repository|Automatically updates your repository to the latest state before tests. Ensures tests are executed on the most up-to-date code.|alpha"
   )
   # ["PMLTC_FLAG_VERBOSE"]="Verbose Output;Displays detailed log messages;alpha"

   [[ ! -f $FLAG_CONFIG_FILE ]] && touch $FLAG_CONFIG_FILE

   load_flags_to_env "$FLAG_CONFIG_FILE"

   while true; do
      clear

      eheader "Flags Menu" 76
      einfo "Toggle flags by selecting their number."

      local options=()
      for flag_entry in "${FLAGS[@]}"; do
         IFS='|' read -r flag name description status <<<"$flag_entry"
         current_value=$(eval "echo \${$flag:-false}")
         checkbox="[ ]"
         [[ "$current_value" == "true" ]] && checkbox="[✔]"

         case $status in
         stable) status_icon="\033[1;32m●\033[0m" ;;
         beta) status_icon="\033[1;33m●\033[0m" ;;
         alpha) status_icon="\033[1;31m●\033[0m" ;;
         esac

         options+=("$checkbox $name $status_icon")
         options+=("    $description")
      done
      ...
      for i in "${!options[@]}"; do
         if ((i % 2 == 0)); then
            eoption "$((i / 2 + 1))) ${options[$i]}" 6
         else
            etext_optionadv cyan normal "${options[$i]}" false 6
         fi
      done

      echo ""
      choice=$(niceprompt "Choose: [1-$((${#FLAGS[@]}))] or ↵ to exit: ")

      [[ -z "$choice" ]] && break
      if [[ "$choice" -ge 1 && "$choice" -le "${#FLAGS[@]}" ]]; then
         flag_entry="${FLAGS[$((choice - 1))]}"
         IFS='|' read -r flag name description status <<<"$flag_entry"
         current_value=$(eval "echo \${$flag:-false}")
         new_value="false"
         [[ "$current_value" == "false" ]] && new_value="true"
         eval "$flag=$new_value"
         export "$flag=$new_value"
         echo -e "$flag = $new_value $FLAG_CONFIG_FILE"
         save_flag_to_config "$flag" "$new_value" "$FLAG_CONFIG_FILE"
      else
         eerror "Invalid choice. Please try again."
      fi
   done

   ecomplete "Flags configuration completed."
}
...
save_flag_to_config() {
   ...
   if grep -q "^$flag=" "$config_file"; then
      "${SED_INPLACE[@]}" "s|^$flag=.*|$flag=$value|" "$config_file"
   else
      echo "$flag=$value" >>"$config_file"
   fi
   esuccess "Saved $flag=$value to $config_file."
}
...
load_flags_to_env() {
   ...
   if [[ -f $config_file ]]; then
      while IFS='=' read -r key value; do
         if [[ $key == PMLTC_FLAG_* ]]; then
            export "$key=$value"
         fi
      done <"$config_file"
      einfohidden "Flags loaded from $config_file into environment variables."
   else
      eerror "Flag configuration file $config_file not found."
   fi
}
...
update_main_menu() {
   ...
   eoption "3) Flags" 6
   eoption "4) Update templates and placeholders" 6
   ...
   choice=$(niceprompt "Choose: [1, 2, 3, 4, reset, $dev_option_label] or ↵ to exit: ")
   echo ""
   case $choice in
   1)
      configure_php_versions
